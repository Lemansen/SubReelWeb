...
const content = {
  RU: {
    ...
    only_win: "Сейчас только для Windows",
  ...
  EN: {
    ...
    only_win: "Currently Windows only",
...
export default function DownloadPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--color-bg)] text-[var(--color-text)] transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 min-h-16 py-3 md:py-0 flex flex-wrap md:flex-nowrap items-center justify-between gap-3 relative">
          ...
          <div className="flex md:hidden items-center gap-2 ml-auto">
            ...
            <button
              ...
              className="p-2 rounded-lg border border-[var(--color-border-sharp)] bg-[var(--color-panel-bg)] text-[var(--color-text)]"
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 w-full">
        ...
        <section className="grid lg:grid-cols-2 gap-10 lg:gap-20 items-center py-12 md:py-16 lg:py-28">
          ...
          <div className="relative z-10 text-center lg:text-left order-2 lg:order-1">
            <div className="inline-flex items-center gap-2 bg-[var(--color-panel-bg)] border border-[var(--color-border-sharp)] rounded-lg px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest text-[var(--color-text-gray)] mb-6">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 shadow-[0_0_6px_rgba(34,197,94,0.7)]" />
              {t.version_info}
            </div>

            <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-[1000] italic uppercase tracking-tighter leading-[0.92] mb-6 md:mb-8">
              {t.hero_title} <br />
              <span className="text-[var(--color-accent-blue)]">{t.hero_title_accent}</span>
            </h1>

            <p className="text-base md:text-lg text-[var(--color-text-gray)] font-medium italic mb-8 max-w-lg leading-relaxed mx-auto lg:mx-0">
              {t.hero_subtitle}
            </p>

            {/* Download buttons — 2-col on mobile */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 justify-center lg:justify-start max-w-sm mx-auto lg:mx-0">
              {os === "Windows" ? (
                <a
                  href="https://github.com/Lemansen/SubReel/releases/download/0.1.3/SubReel.exe"
                  className="group col-span-1 sm:col-span-2 flex items-center justify-between gap-4 bg-[var(--color-accent-blue)] text-white px-6 py-4 rounded-2xl font-black transition-all hover:bg-blue-600 hover:scale-[1.02] active:scale-95 shadow-[0_12px_32px_-8px_rgba(59,130,246,0.5)]"
                >
                  <div className="text-left">
                    <span className="block text-[10px] uppercase opacity-70 tracking-widest mb-0.5">{t.download_btn}</span>
                    <span className="block text-xl leading-none tracking-tighter">WINDOWS</span>
                  </div>
                  <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center group-hover:rotate-12 transition-transform flex-shrink-0">
                    <Download size={22} strokeWidth={3} />
                  </div>
                </a>
              ) : (
                <div className="col-span-1 sm:col-span-2 bg-[var(--color-panel-bg)] border border-[var(--color-border-sharp)] px-6 py-4 rounded-2xl font-black uppercase italic flex items-center justify-center gap-3 opacity-50 text-sm">
                  <Lock size={18} /> {t.only_win}
                </div>
              )}
              <a
                ...
                className="flex items-center justify-center gap-2 border border-[var(--color-border-sharp)] rounded-2xl font-black uppercase italic py-3.5 px-5 hover:bg-[var(--color-panel-hover)] transition-all text-sm text-[var(--color-text-gray)] hover:text-[var(--color-text)]"
              >
                <Settings2 size={16} /> {t.download_unix}
              </a>
              <Link
                href="/server"
                className="flex items-center justify-center gap-2 border border-[var(--color-border-sharp)] rounded-2xl font-black uppercase italic py-3.5 px-5 hover:bg-[var(--color-panel-hover)] transition-all text-sm text-[var(--color-text-gray)] hover:text-[var(--color-text)]"
              >
                <Server size={16} /> {lang === "RU" ? "Сервер" : "Server"}
              </Link>
            </div>
            ...
            <div className="mt-8 flex flex-wrap justify-center lg:justify-start gap-4">
              {[
                { icon: <Zap size={13} />, label: lang === "RU" ? "Быстрый запуск" : "Fast launch" },
                { icon: <Shield size={13} />, label: lang === "RU" ? "Без вирусов" : "Clean & safe" },
                { icon: <Download size={13} />, label: "~12 MB" },
              ].map((s, i) => (
                <div key={i} className="flex items-center gap-1.5 text-[11px] font-bold text-[var(--color-text-gray)] uppercase tracking-wider">
                  <span className="text-[var(--color-accent-blue)]">{s.icon}</span>
                  {s.label}
                </div>
              ))}
            </div>
          ...
          {/* Right: launcher preview */}
          <div className="relative group order-1 lg:order-2">
            <div className="absolute inset-0 bg-[var(--color-accent-blue)]/20 blur-[80px] rounded-full group-hover:bg-[var(--color-accent-blue)]/30 transition-all duration-1000 pointer-events-none" />
            ...
            <div className="relative aspect-[16/10] bg-[#0c0d11] rounded-[1.25rem] md:rounded-[1.5rem] border border-white/5 overflow-hidden shadow-2xl flex font-sans transform lg:rotate-2 group-hover:rotate-0 transition-transform duration-700">
              ...
              <div className="w-[22%] border-r border-white/5 bg-[#0a0b0e] p-4 flex flex-col items-center">
                <div className="w-9 h-9 rounded-xl bg-[var(--color-accent-blue)]/20 border border-[var(--color-accent-blue)]/60 flex items-center justify-center mb-5">
                  <User size={18} className="text-[var(--color-accent-blue)]" />
                </div>
                <div className="w-full space-y-2.5">
                  <div className="h-6 w-full bg-[var(--color-accent-blue)]/10 border-l-2 border-[var(--color-accent-blue)] flex items-center px-3">
                    <div className="h-1 w-10 bg-[var(--color-accent-blue)] rounded-full" />
                  </div>
                  <div className="h-6 w-full flex items-center px-3 opacity-20"><div className="h-1 w-8 bg-white rounded-full" /></div>
                  <div className="h-6 w-full flex items-center px-3 opacity-20"><div className="h-1 w-7 bg-white rounded-full" /></div>
                </div>
              </div>

              {/* Main */}
              <div className="flex-1 p-4 md:p-7 flex flex-col relative">
                ...
                <div className="w-full h-8 bg-white/5 rounded-xl border border-white/5 flex items-center px-3 gap-2 mb-6">
                  <Search size={12} className="opacity-20" />
                  <div className="text-[8px] opacity-20 uppercase font-bold tracking-widest">{t.preview_search}</div>
                </div>

                <div className="grid grid-cols-2 gap-3 flex-1">
                  <div className="bg-white/5 border border-white/10 rounded-xl p-3 flex flex-col items-center justify-center text-center group-hover:border-[var(--color-accent-blue)]/40 transition-colors">
                    <div className="text-[9px] font-black uppercase italic mb-1 tracking-tighter">{t.preview_our_server}</div>
                    <div className="text-[7px] opacity-40 uppercase font-bold">{t.preview_server_desc}</div>
                  </div>
                  <div className="bg-white/5 border border-dashed border-white/10 rounded-xl p-3 flex flex-col items-center justify-center text-center opacity-40">
                    <Plus size={14} className="text-[var(--color-accent-blue)] mb-1" />
                    <div className="text-[9px] font-black uppercase italic tracking-tighter">{t.preview_create}</div>
                  </div>
                </div>

                <div className="mt-4 flex justify-between items-end">
                  <div className="space-y-1">
                    <div className="text-[9px] font-black italic opacity-80">{t.preview_status}</div>
                    <div className="flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                      <div className="text-[7px] font-black uppercase opacity-60 tracking-widest">{t.preview_online}</div>
                    </div>
                  </div>
                  <div className="h-9 px-4 bg-[var(--color-accent-blue)] rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20 group-hover:scale-105 transition-transform gap-1.5">
                    <Play size={11} fill="white" className="text-white" />
                    <span className="text-white font-black italic uppercase text-[10px] tracking-tighter">{t.preview_play}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* ── FEATURES ── */}
        <section className="py-16 md:py-24 border-t border-[var(--color-border-sharp)]">
          <div className="mb-12 md:mb-16 text-center lg:text-left">
            <h2 className="text-3xl md:text-5xl font-black uppercase italic tracking-tighter flex items-center justify-center lg:justify-start gap-4">
              <div className="w-1.5 h-10 bg-[var(--color-accent-blue)] rounded-full hidden lg:block" />
              {t.features_title}
            </h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
            <FeatureCard icon={<Server size={28} />} title={t.feat_server} desc={t.feat_server_desc} color="blue" />
            <FeatureCard icon={<Wrench size={28} />} title={t.feat_builds} desc={t.feat_builds_desc} color="blue" />
            <FeatureCard icon={<Settings2 size={28} />} title={t.feat_custom} desc={t.feat_custom_desc} color="blue" />
            <FeatureCard icon={<Newspaper size={28} />} title={t.feat_news} desc={t.feat_news_desc} color="blue" />
          </div>
        </section>
        ...
        <section className="pb-16 md:pb-24">
          <div className="mb-10 md:mb-14 text-center lg:text-left">
            <h2 className="text-2xl md:text-4xl font-black uppercase italic tracking-tighter text-[var(--color-text-gray)] flex items-center justify-center lg:justify-start gap-4">
              <div className="w-1.5 h-8 bg-[var(--color-text-gray)]/30 rounded-full hidden lg:block" />
              {t.soon_title}
            </h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 opacity-75">
            <FeatureCard icon={<DownloadCloud size={24} />} title={t.soon_mods} desc={t.soon_mods_desc} badge={t.badge_soon} />
            <FeatureCard icon={<Users size={24} />} title={t.soon_social} desc={t.soon_social_desc} badge={t.badge_soon} />
            <FeatureCard icon={<Palette size={24} />} title={t.soon_themes} desc={t.soon_themes_desc} badge={t.badge_soon} />
          </div>
        </section>
      ...
      {/* ── CTA / DISCORD ── */}
      <section className="px-4 md:px-6 py-14 md:py-24 relative overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <div className="relative rounded-[2.5rem] bg-[var(--color-panel-bg)] border border-[var(--color-border-sharp)] overflow-hidden p-7 md:p-16 shadow-2xl">

            <div className="absolute -top-24 -right-24 w-96 h-96 bg-[var(--color-accent-blue)]/15 blur-[120px] rounded-full pointer-events-none" />
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-10 text-center lg:text-left">
              <div className="max-w-2xl">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#5865F2]/10 text-[#5865F2] text-[10px] font-black uppercase tracking-[0.2em] mb-6 border border-[#5865F2]/20">
                  <MessageSquare size={13} fill="currentColor" className="opacity-80" />
                  Community Feedback
                </div>
                <h2 className="text-3xl md:text-5xl font-[1000] uppercase italic tracking-tighter mb-5 leading-[0.95] text-balance">
                  {t.cta_title.split("?")[0]}<span className="text-[var(--color-accent-blue)]">?</span>
                </h2>
                <p className="text-base md:text-lg text-[var(--color-text-gray)] font-medium leading-relaxed">
                  {t.cta_desc}
                </p>
              </div>
              ...
              <div className="flex flex-col items-center gap-5 flex-shrink-0">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group relative inline-flex items-center gap-4 bg-[#5865F2] text-white px-8 py-5 rounded-2xl font-black text-lg transition-all hover:scale-105 hover:shadow-[0_16px_48px_-10px_rgba(88,101,242,0.5)] active:scale-95 overflow-hidden"
                >
                  ...
                  <span className="italic uppercase tracking-tighter relative z-10">{t.cta_btn}</span>
                </a>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex -space-x-2.5">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className="w-8 h-8 rounded-full border-2 border-[var(--color-panel-bg)] bg-[var(--color-bg)] flex items-center justify-center overflow-hidden">
                        <User size={16} className="text-[var(--color-text-gray)]" />
                      </div>
                    ))}
                    <div className="min-w-[32px] px-2 h-8 rounded-full border-2 border-[var(--color-panel-bg)] bg-[var(--color-accent-blue)] flex items-center justify-center text-[9px] font-black text-white">
                      {typeof discordOnline === "number" ? `+${discordOnline}` : "..."}
                    </div>
                  </div>
                  <span className="text-[11px] font-bold text-[var(--color-text-gray)] uppercase tracking-wider">
                    {lang === "RU" ? "в Discord" : "on Discord"}
                  </span>
                </div>
              </div>
            </div>
      ...
      {/* ── FOOTER ── */}
      <footer className="py-10 md:py-14 bg-[var(--color-bg)] border-t border-[var(--color-border-sharp)]">
        <div className="max-w-7xl mx-auto px-4 md:px-6 flex flex-col gap-8">
          <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-8 text-center md:text-left">
            <div className="flex flex-col gap-4 max-w-md">
              <div className="flex items-center justify-center md:justify-start gap-2.5">
                <div className="w-9 h-9 bg-[var(--color-accent-blue)] rounded-xl flex items-center justify-center text-white font-black text-lg italic shadow-md">S</div>
                <span className="font-black text-xl tracking-tighter uppercase italic">Subreel Studio</span>
              </div>
              <p className="text-[11px] uppercase opacity-50 font-bold tracking-widest text-[var(--color-text-gray)] leading-loose">
                {t.footer_disclaimer}
              </p>
            </div>
            <div className="flex flex-wrap justify-center gap-x-8 gap-y-3 text-xs font-black uppercase tracking-widest">
          ...
          <div className="pt-6 border-t border-[var(--color-border-sharp)] text-center">
            <p className="text-[9px] uppercase tracking-[0.4em] font-black text-[var(--color-text-gray)] opacity-50">{t.footer_since}</p>
          </div>
        </div>
      </footer>
    </div>
...
function FeatureCard({
  icon, title, desc, badge, color = "gray",
}: {
  ...
  return (
    <div className="bg-[var(--color-card-bg)] border border-[var(--color-border-sharp)] p-6 md:p-8 rounded-2xl md:rounded-[2rem] hover:border-[var(--color-accent-blue)]/50 hover:shadow-[0_8px_32px_-12px_rgba(59,130,246,0.2)] transition-all group relative overflow-hidden flex flex-col h-full">
      {badge && (
        <div className="absolute top-6 right-6 bg-[var(--color-text-gray)]/10 text-[var(--color-text-gray)] px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-[var(--color-border-sharp)] group-hover:text-[var(--color-accent-blue)] group-hover:bg-[var(--color-accent-blue)]/10 transition-colors">
          {badge}
        </div>
      )}
      <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center mb-6 transition-all group-hover:scale-110 group-hover:-rotate-3 ${badge ? "bg-[var(--color-text-gray)]/10 text-[var(--color-text-gray)]" : "bg-[var(--color-accent-blue)]/10 text-[var(--color-accent-blue)]"}`}>
        {icon}
      </div>
      <h3 className="text-lg md:text-xl font-black uppercase italic mb-3 tracking-tighter group-hover:text-[var(--color-accent-blue)] transition-colors">{title}</h3>
      <p className="text-[var(--color-text-gray)] text-sm font-medium leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity flex-1">{desc}</p>
    </div>
  );
}