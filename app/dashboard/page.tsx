    ...
    <main className="min-h-screen bg-[var(--color-bg)] px-6 py-10 text-[var(--color-text)]">
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
        <section className="rounded-[2rem] border border-[var(--color-border-sharp)] bg-[var(--color-panel-bg)] p-8 shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--color-accent-blue)]">SubReel Account</p>
          ...
          <p className="mt-3 max-w-3xl text-sm text-[var(--color-text-gray)]">
            Это единая точка входа для сайта, лаунчера, предложки, Telegram-подтверждения и будущей экосистемы проекта.
          </p>

          <div className="mt-6 flex flex-wrap gap-3 text-sm">
            <span className="rounded-full border border-[var(--color-border-sharp)] px-4 py-2">login: {user.login}</span>
            <span className="rounded-full border border-[var(--color-border-sharp)] px-4 py-2">nickname: {user.nickname}</span>
            ...
            <span className="rounded-full border border-[var(--color-border-sharp)] px-4 py-2">
              telegram: {user.telegramUserId ? "connected" : "not connected"}
            </span>
          </div>
        </section>
        ...
        <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-5">
          ...
          >
            ...
            <p className="mt-3 text-sm text-[var(--color-text-gray)]">Открывает connect flow и возвращает JWT обратно в лаунчер.</p>
          </Link>
          ...
          >
            ...
            <p className="mt-3 text-sm text-[var(--color-text-gray)]">Идеи, баг-репорты и голосование в одной Supabase-базе.</p>
          </Link>
          ...
          <Link
            ...
            className="rounded-[1.6rem] border border-[var(--color-border-sharp)] bg-[var(--color-panel-bg)] p-6 shadow-[0_18px_60px_rgba(15,23,42,0.08)] transition-transform hover:scale-[1.01]"
          >
            <p className="text-xs font-black uppercase tracking-[0.18em] text-sky-600">Telegram</p>
            <h2 className="mt-3 text-2xl font-black">Привязать бота</h2>
            <p className="mt-3 text-sm text-[var(--color-text-gray)]">Подтверждение аккаунта и безопасные действия через Telegram вместо email.</p>
          </Link>
          ...
          {isStaff ? (
            ...
            >
              ...
              <p className="mt-3 text-sm text-[var(--color-text-gray)]">Разбор идей и багов, смена статусов и публикация approved-контента.</p>
            </Link>
          ) : (
            <div className="rounded-[1.6rem] border border-dashed border-[var(--color-border-sharp)] bg-[var(--color-panel-bg)] p-6 text-sm text-[var(--color-text-gray)]">
              ...
              <p className="mt-3">Этот раздел доступен только для ролей moderator и admin.</p>
            </div>
          )}
